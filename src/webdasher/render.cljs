...
(defn render-board [state]
  [:div#board {:style {:height (@scenario :board-height) :width (@scenario :board-width)}}
   [render-lanes state]])

(def interesting-keys [:status :score :speed])
